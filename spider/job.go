...
import (
	...
	"lagou_jobs/pipeline"
	"lagou_jobs/pkg/convert"
	"lagou_jobs/pkg/page"
	"lagou_jobs/scheduler"
	"log"
	"time"
)

var (
	delayTime = time.Tick(time.Microsecond * 1000)
...
func GetJobs(city string, pn int, kd string) ([]downloader.Result, int, int, error) {
	...
	if result.Code == 0 && result.Success == true {
		...
		if content.PositionResult.TotalCount > 0 && content.PageSize > 0 {
			totalPage = page.CaculateTotalPage(float64(content.PositionResult.TotalCount), float64(content.PageSize))
		}
	} else {
